---
layout: default
title: About Me
permalink: /about/
---

<div style="display: flex; align-items: center; margin-bottom: 2rem;">
  <img src="/assets//images/profile.jpg" alt="Profile Picture" style="border-radius: 50%; width: 150px; height: 150px; margin-right: 20px;">
  <div>
    <h2>Seàn</h2>
    <p>Data Scientist | Designer | Problem Solver</p>
  </div>
</div>

## Purpose of This Portfolio

This portfolio showcases my journey and the projects I have undertaken to develop my skills in data science. It highlights my ability to **analyze**, **visualize**, and **communicate** data effectively, demonstrating my commitment to continuous learning and professional growth. 

You can read more about me in the tabs below:
{% tabs key_sections %}

{% tab key_sections profile %}
## Professional Profile

I am a versatile problem-solver with a passion for **design**, **data**, and **technology**. With 18 years of self-taught design experience and a strong background in **project management**, **organisational development**, **design**, and **data analysis**, I specialize in bridging the gap between business needs and technology solutions. My career spans international leadership roles, strategic planning, and data-driven decision-making, making me a well-rounded professional with a unique perspective.

My goal is to leverage my technical expertise, design skills, and international experience to contribute to innovative business solutions. I am passionate about using data to drive meaningful decisions, solve complex problems, and create a positive impact.

Below is a visual representation of my international career journey:

<iframe src="{{ '/assets/html/interactive_map.html' }}" style="width: 100%; height: 60vh; overflow-y: scroll; border: none;"></iframe>

<!--![Work Map](/assets/images/work_map.png)-->

<hr>
{% endtab %}


{% tab key_sections expertise %}
## Key Skills and Expertise
### Data Analysis & Visualization:
- Proficient in **R**, **Python**, **SQL**, **Excel**, **PowerBI**, and **Tableau**; with a focus on transforming complex data into actionable insights.

### Stakeholder Management:
- Extensive experience managing cross-sector collaborations, including raising **over €500,000** for a satellite project in the space industry.

### Project Leadership:
- Led teams in **Kazakhstan**, **Krygzstan**, **Myanmar**, and **Scotland**; developing expertise in strategic planning, budgets, and project finance.

### Consulting:
- Supported clients with ERP systems, training programmes, and data-driven insights, simplifying complex data into clear, actionable strategies.

### Design & Innovation:
- 18 years of self-taught design experience, enabling me to create visually compelling and user-friendly solutions. Proficient in the **Adobe** Suite programs, **Affinity Designer** and **Publisher**, OpenCanvas, and some experience with 3d visualisation tools including **Blender** and **3DSMax**. 

### Continuous Learning:
- I am constantly building my skills, learning languages like **Polish** and **Scottish Gaelic**, while expanding my knowledge of cloud platforms such as **AWS** and **Azure**. I'm also exploring generative coding and creative projects using **Three.js**, **p5.js**, and other innovative tools for artistic expression and interactive development. Additionally, I am learning how to utilise ArcGIS software. 

<hr>
{% endtab %}

{% tab key_sections career %}
## Career Highlights

### CEO & Director of Organisational Development – Leadership Development NGO, Central Asia
- Led strategic initiatives to enhance organizational capacity and sustainability.
- Built high-performing teams and fostered a culture of collaboration and innovation.
- Secured funding and strengthened partnerships with donors and local communities.
- Designed leadership training programs and established impact measurement frameworks.

### Space Industry & Data Science
After seven years in the Space sector, including roles as a business consultant, I transitioned into data science in 2021. This shift aligns with my lifelong passions for art, design, coding, systems, statistics, and data. My experience in the space industry, particularly in facilitating cross-sector collaborations and securing funding for innovative projects, reinforced my belief in the power of data as a decision-making tool.

### Recent Work
Over the past three years, I have worked as a data analyst and consultant, supporting clients with diverse needs—from designing workshops to analysing and visualising complex datasets. My adaptability and international experience have equipped me to deliver innovative solutions in dynamic environments.

<hr>
{% endtab %}


{% tab key_sections skills  %}
## Skills
### Languages
- English: Native
- Russian: B1
- Ukrainian: A1
- Polish: A1
- Gàidhlig: A2
- *Levels: A1 and A2: Basic user; B1 and B2: Independent user, C1 and C2: Proficient user*

### Data Science

- Business Intelligence, Business Analytics, Business Synthesis  
- Data Collection, Processing, Analysis, and Visualization  
- Data Visualization Tools: Tableau, Power BI, R (RStudio), ArcGIS (Geographic Information System), Positron  
- Visualizations: Graphs, Maps, Infographics  
- Statistical Analysis: Hypothesis Testing, A/B Testing, Regression, Bayesian Statistics  
- Time Series Analysis  
- Data Ethics & Privacy: GDPR Compliance, Data Security Best Practices  

### Programming Languages

- c#, C++, Java, JavaScript, Python, SQL, R, uScript, Three.js, Jekyll, HTML  

### Art & Design

- Software: Affinity Photo, Affinity Designer, Adobe Suite (Photoshop, Illustrator, After Effects, Lightroom), openCanvas  
- Functional Design: Maya, 3ds Max, 2D/3D Drawing  
- Microsoft PowerPoint  
- Video and Photography Editing  
- Autodesk  

### Project Management

- Tools: Trello, Hive, Microsoft Project, Excel, etc.  
- Project Plans and Reports  
- Resource, Budget, and Cost Optimization  
- Risk Assessment and Management  
- Development Methodologies: Scrum, Kanban  
- Cross-departmental Coordination  

### Writing & Editing

- Affinity Publisher  
- Content Creation: Blog Posts, Social Media Content, Podcasts  
- Copywriting  
- Microsoft Office: Word, Excel (advanced - VlookUp, Pivot tables, power query), PowerPoint, Access  
- Grant Applications  

### Version Control

- Git, GitHub, GitLab  
- Branching Strategies  
- Code Collaboration and Reviews  

### Miscellaneous

- Event Organization, Conference Facilitation  
- Session Design, Delivery, and Management  
- English Language Teaching  
- Basic Video Game Design and Tools (Unreal Engine)  
- Speaking and Public Relations  
- Level 6 Digital Skills Test  
- Teamwork, Organizational, and Planning Skills  
- Research and Analytical Skills  
- Cross-department Collaboration  
- Communication Skills  
- Student Engagement  
- B2B and B2C Sales  


<hr>
{% endtab %}

{% tab key_sections education  %}
## Education and Training

### GIS for Climate Action
- Institute: Esri
- Date: 19/02/2025 – 30/03/2025
- Location: Warsaw, Poland
- Academic level: MOOC
- Field of study:  Data Science with ArcGIS Pro

### DATA SCIENCE BOOTCAMP
- Institute: CodeClan 
- Date: 03/11/2021 – 05/03/2022
- Location: Edinburgh, United Kingdom
- Academic level: SQA PDA 8 // EQF Level 5
- Field of study:  Inter-disciplinary programmes and qualifications involving natural sciences, mathematics and statistics

### INTRO TO DATA SCIENCE: PYTHON
- Institute: The University of Glasgow [www.gla.ac.uk](https://www.gla.ac.uk)
- Date: 01/05/2022 – 10/06/2022
- Location: Glasgow, United Kingdom
- Academic level: Postgraduate Microcredential
- Field of study:  Data Science

### APPLIED DATA SKILLS: Processing & Presenting Data
- Institute: The University of Glasgow [www.gla.ac.uk](https://www.gla.ac.uk)
- Date: 12/05/2021 – 05/07/2021
- Location: Glasgow, United Kingdom
- Academic level: Postgraduate Microcredential
- Field of study:  Data Science

### PRACTICAL PROJECT MANAGEMENT
- Institute: The University of Glasgow [www.gla.ac.uk](https://www.gla.ac.uk)
- Date: 02/02/2021 – 29/04/2021
- Location: Glasgow, United Kingdom
- Academic level: Postgraduate Microcredential
- Field of study:  Project Management

### SOFTWARE DEVELOPMENT FUNDAMENTALS
- Institute: University of Highlands & Islands \ Oilthigh na Gàidhealtachd agus nan  [www.uhi.ac.uk](https://www.uhi.ac.uk)
- Date: 04/02/2021 – 10/06/2021
- Location: Moray, United Kingdom
- Academic level: Postgraduate Microcredential
- Field of study:  Software Development

### COPERNICUS MOOC
- Institute: University of Luxembourg [https://www.uni.lu](https://www.uni.lu)
- Date: 10/07/2020 – 17/12/2020
- Location: Luxembourg, Luxembourg 
- Academic level: MOOC
- Field of study:  GIS with COPERNICUS satellite data

### EC's SPACE ACADEMY
- Institute: European Commission [Space Academy](https://cordis.europa.eu/project/id/776356)
- Date: 03/09/2019 – 10/09/2019
- Location: Helsinki, Finland
- Academic level: N/A
- Field: Entrepreuneurship in Space Industry 

### TRAIN THE TRAINERS (TtT)
- Institute: AIESEC UK [aiesec.co.uk](https://www.aiesec.co.uk)
- Date: 05/06/2012 – 10/06/2012 
- Location: Edinburgh, United Kingdom 
- Academic level: N/A
- Field: Public Speaking / Conference Management / Session Design


<hr>
{% endtab %}
{% endtabs %}


## Accessibility Note

As a neurodivergent individual and a refugee from Ukraine, I bring a unique perspective and resilience to my work in data science. I am committed to leveraging my skills to solve complex problems and contribute to meaningful projects
